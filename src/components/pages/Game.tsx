...
export default function Game() {
  ...
  const makeApiMove = async (moveData: any) => {
    if (!gameId) return;
    
    setSubmittingMove(true);
    try {
      const requestId = uuidv4();
      const expectedVersion = useGameStore.getState().streamVersion ?? 0;
      await gamesAPI.makeMove(gameId, moveData, requestId, expectedVersion);
    } catch (error: any) {
      setGameError(error.response?.data?.error || 'Failed to make move');
    } finally {
  ...
  useEffect(() => {
    ...
    const join = async () => {
      try {
        const requestId = uuidv4();
        const expectedVersion = useGameStore.getState().streamVersion ?? 0;
        await gamesAPI.joinGame(String(gameId), requestId, expectedVersion);
      } catch (error: any) {
        setGameError(error.response?.data?.error || 'Failed to join game');
      }
    };
    join();
    ...
    return () => {
      ...
      gamesAPI.leaveGame(String(gameId));
