...
import { MoveType, GamePhase } from '@gin-rummy/common';
...
export default function Game() {
  ...
  useEffect(() => {
    ...
    if (myPlayer?.hand) {
      const currentCardIds = myPlayer.hand.map(card => card.id);
      ...
      if (!handOrder.length) {
        ...
        const existingCards = handOrder.filter(cardId => currentCardIds.includes(cardId));
        const newCards = currentCardIds.filter(cardId => !handOrder.includes(cardId));
  ...
  const getDeadwoodAfterDiscard = (cardId: string): number => {
    ...
    const handWithoutCard = myPlayer.hand.filter(card => card.id !== cardId);
    
    // For now, use current melds calculation - this could be enhanced to recalculate optimal melds
    const meldsWithoutCard = myPlayer.melds?.filter(meld => 
      !meld.cards.some(card => card.id === cardId)
    ) || [];
    
    // Calculate deadwood with remaining melds
    const meldedCardIds = new Set(
      meldsWithoutCard.flatMap(meld => meld.cards.map(card => card.id))
    );
    
    const deadwoodCards = handWithoutCard.filter(card => !meldedCardIds.has(card.id));
        ...
        <div className="bg-white rounded-lg shadow-sm p-4 mb-4">
          <div className="flex justify-between items-center">
            ...
            <div className="text-right">
              <div className="text-sm text-gray-600">Phase: {getPhaseDisplayText(gameState.phase)}</div>
